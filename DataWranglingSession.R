...
df <- rename(df, 
             "work" = "job", 
             "schooling" = "education", 
             "sex" = "gender") # It is perfect but if you run it will give an error because the column names have been changed and so will act as though the original names were not there but if you call the data new names wil appear
names(df)

# Filter() function
# The function lets you extract the rows from a data frame based on logical criteria.
# Arguments:
# 1. Data
# 2. Logical tests for the rows to be kept.
# Can also use the below to create more complex logic; &(AND), |(OR) 
dplyr:: filter(df, work == "admin")
dplyr:: filter(df, work == "admin" & sex == "female")
dplyr:: filter(df, work == "admin" | sex == "male")

# filtering can also be used to clean data, removing unwanted and suscipicious data, or even outliers
